    ...
    <div style={{ minHeight: '100vh', padding: '40px 24px' }}>
      <div style={{ maxWidth: '900px', margin: '0 auto' }}>
        ...
        <div style={{ marginBottom: '36px' }}>
          <p style={{ color: '#F39F5A', fontSize: '14px', fontWeight: 600, marginBottom: '6px' }}>
            Good to see you 👋
          </p>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '20px', marginBottom: '28px' }}>
          {[
            { value: '73', label: 'Profile Score', icon: '📊' },
            { value: '12', label: 'Proposals Generated', icon: '✍️' },
            { value: '3', label: 'Sent Today', icon: '📤' },
          ].map((stat, i) => (
            <div key={i} className="card" style={{ textAlign: 'center', padding: '24px' }}>
              <div style={{ fontSize: '28px', marginBottom: '10px' }}>{stat.icon}</div>
              <p className="gradient-text" style={{ fontSize: '42px', fontWeight: 800, margin: '0 0 4px' }}>
                {stat.value}
              </p>
              <p style={{ color: '#E8BCB9', fontSize: '14px', opacity: 0.7 }}>{stat.label}</p>
            </div>
          ))}
        </div>
        ...
        <div className="card">
          ...
          {[
            ...
            <div key={index} style={{
              ...
              borderBottom: index < 2 ? '1px solid rgba(232,188,185,0.1)' : 'none'
            }}>
              <div>
                <p style={{ fontWeight: 600, color: 'white', marginBottom: '6px' }}>{proposal.title}</p>
                <span style={{
                  fontSize: '12px', padding: '3px 12px', borderRadius: '999px', fontWeight: 500,
