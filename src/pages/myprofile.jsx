    ...
    <div style={{ minHeight: '100vh', padding: '40px 24px' }}>
      <div style={{ maxWidth: '760px', margin: '0 auto' }}>
        ...
        <div className="card" style={{ marginBottom: '20px', display: 'flex', alignItems: 'center', gap: '24px', flexWrap: 'wrap' }}>
          <div style={{ width: '80px', height: '80px', borderRadius: '50%', flexShrink: 0, background: 'linear-gradient(135deg, #F39F5A, #AE445A)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '32px', fontWeight: 800, color: 'white', boxShadow: '0 0 0 4px rgba(243,159,90,0.2)' }}>
            {avatarLetter}
          </div>
          <div style={{ flex: 1 }}>
            <h2 style={{ fontSize: '24px', fontWeight: 800, color: 'white', margin: '0 0 4px' }}>{profile.full_name}</h2>
          ...
          <button onClick={() => navigate('/profile')} className="btn-primary" style={{ padding: '10px 20px', fontSize: '14px', whiteSpace: 'nowrap' }}>Analyze Profile </button>
        </div>

        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(340px, 1fr))', gap: '20px', marginBottom: '20px' }}>
          <div className="card">
            <h3 style={{ fontSize: '14px', fontWeight: 700, color: '#F39F5A', letterSpacing: '1px', margin: '0 0 16px' }}>CONTACT INFO</h3>
          ...
          <div className="card">
            <h3 style={{ fontSize: '14px', fontWeight: 700, color: '#F39F5A', letterSpacing: '1px', margin: '0 0 16px' }}>TOP SKILLS</h3>
            {skillTags.length > 0 ? (
        ...
        {profile.bio && (
          <div className="card" style={{ marginBottom: '20px' }}>
            <h3 style={{ fontSize: '14px', fontWeight: 700, color: '#F39F5A', letterSpacing: '1px', margin: '0 0 12px' }}>ABOUT ME</h3>
            <p style={{ color: '#E8BCB9', opacity: 0.8, lineHeight: 1.8, fontSize: '15px', margin: 0 }}>{profile.bio}</p>
          </div>
        )}

        <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
          <button onClick={() => navigate('/dashboard')} style={{ padding: '11px 22px', borderRadius: '12px', fontWeight: 600, fontSize: '14px', border: '1.5px solid rgba(232,188,185,0.2)', background: 'transparent', color: '#E8BCB9', cursor: 'pointer', transition: 'all 0.2s' }}
            onMouseEnter={e => e.currentTarget.style.background = 'rgba(255,255,255,0.06)'}
            onMouseLeave={e => e.currentTarget.style.background = 'transparent'}>
            ← Back to Dashboard
          </button>
          <button onClick={() => navigate('/proposal')} className="btn-primary" style={{ padding: '11px 22px', fontSize: '14px' }}>Generate a Proposal →</button>
        </div>
